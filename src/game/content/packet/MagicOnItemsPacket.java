...
public class MagicOnItemsPacket implements PacketType
{
		...
		public void processPacket(Player player, int packetType, int packetSize, boolean trackPlayer)
		{
				...
				boolean staff = ItemAssistant.getItemName(player.getWieldedWeapon()).toLowerCase().contains("staff") ? true : false;
				...
				switch (spellId)
				{
						...
						case 15877:
								...
								if ((ItemAssistant.hasItemAmountInInventory(player, 555, 4) || ItemAssistant.hasItemEquipped(player, 1383)) && (ItemAssistant.hasItemAmountInInventory(player, 557, 4) || ItemAssistant.hasItemEquipped(player, 1385)) && ItemAssistant.hasItemAmountInInventory(player, 561, 2))
								{
						...
						case 1162:
								...
								if (System.currentTimeMillis() - player.alchDelay > 1000)
								{
										...
										if (itemId == 995)
										{
												player.playerAssistant.sendMessage("You can't alch coins!");
												break;
										}
										...
										{
												if (ItemAssistant.hasSingularUntradeableItem(player, Integer.parseInt(player.singularUntradeableItemsOwned.get(index))))
												{
														player.singularUntradeableItemsOwned.remove(index);
												}
										}
										...
										Skilling.addSkillExperience(player, CombatConstants.MAGIC_SPELLS[49][1], 6);
						...
						case 1178:
								...
								if (System.currentTimeMillis() - player.alchDelay > 2000)
								{
										...
										player.startAnimation(staff ? 9633 : 713);
										player.gfx0(staff ? 1693 : 1691);
										player.alchDelay = System.currentTimeMillis();
										player.getPA().sendFrame106(6);
										Skilling.addSkillExperience(player, CombatConstants.MAGIC_SPELLS[50][1], 6);
										Skilling.updateSkillTabFrontTextMain(player, ServerConstants.MAGIC);
								}
								break;
						// Superheat.
