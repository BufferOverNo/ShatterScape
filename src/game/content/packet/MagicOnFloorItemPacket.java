...
public class MagicOnFloorItemPacket implements PacketType
{
		...
		public void processPacket(final Player player, int packetType, int packetSize, boolean trackPlayer)
		{
				...
				if ((((ItemAssistant.getFreeInventorySlots(player) >= 1) || ItemAssistant.hasItemAmountInInventory(player, itemId, 1)) && ItemDefinition.getDefinitions()[itemId].stackable) || ((ItemAssistant.getFreeInventorySlots(player) > 0) && !ItemDefinition.getDefinitions()[itemId].stackable))
				{
						if (player.playerAssistant.withInDistance(player.getX(), player.getY(), itemX, itemY, 12))
						{
								...
								player.startAnimation(CombatConstants.MAGIC_SPELLS[51][2]);
								player.gfx100(CombatConstants.MAGIC_SPELLS[51][3]);
								player.getPA().createPlayersStillGfx(144, itemX, itemY, 0, 72);
								player.getPA().createPlayersProjectile(player.getX(), player.getY(), offX, offY, 50, 70, CombatConstants.MAGIC_SPELLS[51][4], 50, 10, 0, 50, Combat.getProjectileSlope(player));
								Skilling.addSkillExperience(player, CombatConstants.MAGIC_SPELLS[51][7], 6);
								...
								CycleEventHandler.getSingleton().addEvent(player, new CycleEvent()
								{
										...
										public void execute(CycleEventContainer container)
										{
												...
												if (System.currentTimeMillis() - player.teleGrabDelay > 1100 && player.isUsingMagic() && player.walkingToItem)
												{
														if (Server.itemHandler.itemExists(player.teleGrabItem, player.teleGrabX, player.teleGrabY) && player.playerAssistant.withInDistance(player.getX(), player.getY(), itemX, itemY, 12))
														{
