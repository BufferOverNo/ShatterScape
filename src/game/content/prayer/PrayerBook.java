...
public class PrayerBook
{
		...
		public static void resetAllPrayerGlows(Player player)
		{
				for (int p = 0; p < player.prayerActive.length; p++)
				{
						...
						player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[p], 0);
				}
		}
		...
		public static void activatePrayer(Player player, int prayerIndex)
		{
				...
				{
						player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[prayerIndex], player.prayerActive[prayerIndex] ? 1 : 0);
						sound = 447;
				...
				if (System.currentTimeMillis() - player.getTimeUnderAttackByAnotherPlayer() <= 9000 || System.currentTimeMillis() - player.getTimeAttackedAnotherPlayer() <= 9000)
				{
						if (prayerIndex >= 21 && QuickSetUp.isUsingF2pOnly(player, false, false) && !player.prayerActive[prayerIndex])
						{
								...
								player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[prayerIndex], 0);
								sound = 447;
								SoundSystem.sendSound(player, sound, 0);
								return;
						}
				}
				if (prayerIndex == ServerConstants.RETRIBUTION && player.getHeight() == 20 || player.getHeight() == 20 && prayerIndex >= 16 && prayerIndex <= 18 && !Tournament.eventType.equals("Pure tribrid") && !Tournament.eventType.equals("Main Nh"))
				{
				...
				if (prayerIndex == ServerConstants.PROTECT_ITEM && System.currentTimeMillis() - player.protectItemDisabledTime <= 600000)
				{
						player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[prayerIndex], 0);
						long time = (player.protectItemDisabledTime + 600000) - System.currentTimeMillis();
						time /= 60000;
						if (time == 0)
						{
								time = 1;
						}
						String second = time > 1 ? "minutes" : "minutes";
						player.getDH().sendStatement("You cannot use protect item for " + time + " " + second + ".");
						sound = 447;
						SoundSystem.sendSound(player, sound, 0);
						return;
				}
				...
				int[] defPray = {0, 5, 13, 24, 25, ServerConstants.RIGOUR, ServerConstants.AUGURY};
				int[] strPray = {1, 6, 14, 24, 25};
				int[] atkPray = {2, 7, 15, 24, 25};
				int[] rangePray = {3, 11, 19, ServerConstants.RIGOUR};
				int[] magePray = {4, 12, 20, ServerConstants.AUGURY};
				if (player.getCurrentCombatSkillLevel(ServerConstants.PRAYER) > 0)
				{
						if (player.baseSkillLevel[ServerConstants.PRAYER] >= ServerConstants.PRAYER_LEVEL_REQUIRED[prayerIndex])
						{
								...
								switch (prayerIndex)
								{
												...
												if (player.prayerActive[prayerIndex] == false)
												{
														for (int j = 0; j < defPray.length; j++)
														{
																if (defPray[j] != prayerIndex)
																{
																		...
																		player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[defPray[j]], 0);
																}
														}
												}
												break;
												...
												if (player.prayerActive[prayerIndex] == false)
												{
														for (int j = 0; j < strPray.length; j++)
														{
																if (strPray[j] != prayerIndex)
																{
																		...
																		player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[strPray[j]], 0);
																}
														}
														for (int j = 0; j < rangePray.length; j++)
														{
																if (rangePray[j] != prayerIndex)
																{
																		...
																		player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[rangePray[j]], 0);
																}
														}
														for (int j = 0; j < magePray.length; j++)
														{
																if (magePray[j] != prayerIndex)
																{
																		...
																		player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[magePray[j]], 0);
																}
														}
												}
												break;
												...
												if (player.prayerActive[prayerIndex] == false)
												{
														for (int j = 0; j < atkPray.length; j++)
														{
																if (atkPray[j] != prayerIndex)
																{
																		...
																		player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[atkPray[j]], 0);
																}
														}
														for (int j = 0; j < rangePray.length; j++)
														{
										...
										case 18:
												if (System.currentTimeMillis() - player.stopPrayerDelay < 5000)
												{
														...
														player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[16], 0);
														player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[17], 0);
														player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[18], 0);
														sound = 447;
														SoundSystem.sendSound(player, sound, 0);
														return;
												}
								...
								if (!headIcon)
								{
										if (player.prayerActive[prayerIndex] == false)
										{
												...
												player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[prayerIndex], 1);
												switch (prayerIndex)
												{
														case ServerConstants.THICK_SKIN:
																sound = 446;
						...
						{
								player.getPA().sendFrame36(ServerConstants.PRAYER_GLOW[prayerIndex], 0);
								player.getPA().sendFrame126("You need a Prayer level of " + ServerConstants.PRAYER_LEVEL_REQUIRED[prayerIndex] + " to use " + ServerConstants.PRAYER_NAME[prayerIndex] + ".", 357);
		...
		public static void handlePrayerDrain(final Player player)
		{
				...
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent()
				{
						...
						public void execute(CycleEventContainer container)
						{
								...
								if (prayerAmountToRemoveTotal <= 0.014) // Keep it at this variable or the event won't stop.
								{
										container.stop();
										return;
								}
						...
						public void stop()
						{
								player.timePrayerActivated = new long[player.prayerActive.length];
