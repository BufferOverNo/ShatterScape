...
//				System.out.println(itemId);
	...
	public void checkShopPrice(int itemId, int removeSlot)
	{
		...
		player.playerAssistant.sendMessage(ItemAssistant.getItemName(itemId) + ": currently costs " + (Misc.formatNumber(getItemShopValue(itemId))) + " coins.");
	...
	public void buyItem(int itemId, int fromSlot, int amount)
	{
		...
		int itemCost = getItemShopValue(itemId);
		...
		if (ItemAssistant.getItemAmount(player, 995) >= itemCost) {
			ItemAssistant.deleteItemFromInventory(player, 995, itemCost);
		} else {
			player.playerAssistant.sendMessage("Not enough gold to purchase this.");
			return;
		}
		ItemAssistant.addItem(player, itemId, amount);
		ItemAssistant.resetItems(player, 3823);
	...
	public void priceCheckItemToSell(Player player, int itemId)
	{
		...
		player.playerAssistant.sendMessage(ItemAssistant.getItemName(itemId) + ": can be sold for " + Misc.formatNumber(value) + " coins.");
	}
	...
	public void sellItemToShop(Player player, int itemId, int itemSlot, int amount)
	{
		...
		int price = getSellToShopPrice(itemId);
		...
		if (parse[1].equals("true"))
		{
			...
			{
				price = BloodMoneyPrice.getDefinitions()[ItemAssistant.getUnNotedItem(itemId)].harvestedPrice;
		...
		if (player.getHeight() != 20) {
			ItemAssistant.addItemToInventoryOrDrop(player, 995, price * amount);
		}
