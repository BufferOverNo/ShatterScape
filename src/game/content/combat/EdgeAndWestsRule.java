...
public class EdgeAndWestsRule
{
		 ...
		 */
		public final static int TIME_IMMUNE_FROM_BEING_ATTACKED = 30;

		/**
		 * How much blood money risked is needed to go under the protection.
		 */
		public final static int BLOOD_MONEY_RISK_REQUIRED_FOR_PROTECTION = 10000;


		public final static int MAXIMUM_EXCESS_BREW_DOSES = 20;



		/**
		 * @return
		 * 	True if the rules of Edgeville wilderness applies to the player.<p>
		...
		public static boolean isEdgeOrWestRule(Player attacker, Player victim, String action)
		{
				...
				if (Area.inWilderness(attacker) || Area.inWilderness(victim))
				{
						if (!attacker.isBot)
						{
						...
						if (attacker.wildernessRiskAmount < 20000)
						{
								if (victim.wildernessRiskAmount >= BLOOD_MONEY_RISK_REQUIRED_FOR_PROTECTION && attacker.wildernessRiskAmount < victim.wildernessRiskAmount / 5 && !Area.inMulti(victim.getX(), victim.getY()))
								{
										int riskNeeded = (victim.wildernessRiskAmount / 5);
										if (riskNeeded > 20000)
										{
												riskNeeded = 20000;
										}
				...
				switch (action)
				{
						case "NORMAL ATTACK":
								...
								*/
								...
								/*
								if (victim.excessBrews && !attacker.excessBrews)
								{
										reasons.add("has 2 brews or more");
								}
								*/

								/*
								if (attacker.wildernessRiskAmount >= BLOOD_MONEY_RISK_REQUIRED_FOR_PROTECTION && victim.wildernessRiskAmount < attacker.wildernessRiskAmount / 3)
								{
										reasons.add("risking " + victim.wildernessRiskAmount + " blood money");
								}
								*/
								...
								if (attackerHasF2p && !victimHasF2p)
								{
										reasons.add("is not wearing full f2p");
								}
		...
		public static void edgeOrWestRuleCoordinateUpdate(Player victim, boolean logInTeleportCompleted)
		{
				...
				else if (Area.inWilderness(victim))
				{
		...
		*/
		...
		public static boolean isTribrid(Player player)
		{
				...
				{
						if (ItemAssistant.hasItemInInventory(player, 560) || ItemAssistant.hasItemInInventory(player, 18820) && RunePouch.specificRuneInsideRunePouch(player, "CHECK", 560, 2))
						{
								combatStylesUsed++;
				...
				if (EdgeAndWestsRule.isUnderEdgeAndWestsProtectionRules(player))
				{
						...
						if (System.currentTimeMillis() - player.getTimeAttackedAnotherPlayer() <= 8000 || System.currentTimeMillis() - player.getTimeUnderAttackByAnotherPlayer() <= 8000)
						{
								if (pickup)
								{
										...
										{
												if (player.combatStylesUsed <= 2)
												{
														if (RangedData.isRangedItem(player, itemId))
														{
																player.getPA().sendMessage("You cannot pickup a Ranged item under Edgeville protection.");
																return false;
														}
												}
										}
		...
		public static boolean canPickUpOrReceiveBrew(Player player, int itemId)
		{
				...
				if (Area.inWilderness(player))
				{
						if (ItemDefinition.getDefinitions()[itemId].name.contains("Saradomin brew") && !ItemDefinition.getDefinitions()[itemId].note)
						{
								...
								{
										player.getPA().sendMessage("You cannot have more than 10 brews at a time.");
										return false;
								}
						}
				}
		...
		*/
		...
		/**
		 * If the player teletabs, the enemy gets 500 blood money.
		 * @param victim
		 */
		public static void teleTab(Player victim)
		{
				if (System.currentTimeMillis() - victim.getTimeUnderAttackByAnotherPlayer() <= 8000)
				{
						...
						if (attacker != null)
						{
								ItemAssistant.addItemToInventoryOrDrop(attacker, 18644, 400);
								attacker.getPA().sendMessage(victim.getPlayerName() + " tabbed from you, you are awarded 400 Blood money.");
								attacker.enemyTabs++;
		...
		/**
		 * @return
		 * 	True if the player can process to next destination with the amount of brews the player has.
		 */
		public static boolean canProcessToDestinationWithBrews(Player player, int destinationX, int destinationY)
		{
				if (!Area.inWilderness(player))
				{

						if (Area.inWilderness(destinationX, destinationY))
						{
								...
								{
										player.getPA().sendMessage("Your may only carry a maximum of 10 brews to the wilderness.");
										return false;
								}
