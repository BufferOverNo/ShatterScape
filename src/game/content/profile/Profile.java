...
public class Profile
{
		 ...
		 * @return
		 * 			True, if the button belongs to the profile interface.
		 */
		public static boolean isProfileButton(Player player, int button)
		{
				...
				switch (button)
				{
						...
						case 109100:
								...
								{
										player.getPA().setInterfaceClicked(25332, true);
								}
						...
						case 98248:
								tabSelected = "PKING";
								player.getPA().setInterfaceClicked(25336, true);
								break;

						case 98252:
								tabSelected = "SKILLING";
								player.getPA().setInterfaceClicked(25340, true);
								break;

						case 99000:
								tabSelected = "PVM";
								player.getPA().setInterfaceClicked(25344, true);
								break;

						case 99004:
								tabSelected = "MISC";
								player.getPA().setInterfaceClicked(25348, true);
								break;
				}
		...
		public static void viewCorrectTab(Player player, String tabText, boolean forceOfflineSearch)
		{
				...
				{
						if (searched.gameMode.equals("Bot"))
						{
								player.getPA().sendMessage(Misc.capitalize(player.getProfileNameSearched()) + " is a bot, profile does not exist.");
								return;
						}
		...
		public static void viewInfoPage(Player player)
		{
				...
				Player searched = PlayerHandler.players[player.getProfileSearchOnlinePlayerId()];
				rank = PlayerRank.getIconText(rank, player.isProfileSearchOnline ? searched.playerRights : ProfileSearch.playerRights, true) + PlayerRank.getRankName(rank, player.isProfileSearchOnline ? searched.playerRights : ProfileSearch.playerRights);
				if (rank.isEmpty())
				{
						rank = "None";
				}
				player.getPA().sendFrame126("Server rank: " + rank, 25353);
				player.getPA().sendFrame126("Game mode: " + Misc.capitalize(player.isProfileSearchOnline ? searched.gameMode : ProfileSearch.gameMode), 25354);
				...
				player.getPA().sendFrame126("Wilderness kills: " + (player.isProfileSearchOnline ? searched.wildernessKills : ProfileSearch.wildernessKills), 25357);
				...
				player.getPA().sendFrame126("Time spent: " + GameTimeSpent.getPercentagePlayed(player.isProfileSearchOnline ? searched.timeSpent : ProfileSearch.timeSpent, GameTimeSpent.PKING) + "% Pk, " + GameTimeSpent.getPercentagePlayed(player.isProfileSearchOnline ? searched.timeSpent : ProfileSearch.timeSpent, GameTimeSpent.SKILLING) + "% Skill, " + GameTimeSpent.getPercentagePlayed(player.isProfileSearchOnline ? searched.timeSpent : ProfileSearch.timeSpent, GameTimeSpent.PVM) + "% Pvm", 25371);
				ProfileRank.applyRank(player, "PLAYER KILLING");
				ProfileRank.applyRank(player, "ADVENTURER");
				player.getPA().displayInterface(25320);
		}

		private final static int[] SKILL_TAB_SKILL_ID_ORDER = {0, 2, 1, 4, 5, 6, 20, 3, 16, 15, 17, 12, 9, 18, 14, 13, 10, 7, 11, 8, 19};
		...
		public static void viewPvmPage(Player player)
		{
				...
				player.getPA().sendFrame126("Monster kills: " + NpcKillTracker.getNpcAmount(player.isProfileSearchOnline ? searched.npcKills : ProfileSearch.npcKills, "NORMAL") + ", Boss kills: " + NpcKillTracker.getNpcAmount(player.isProfileSearchOnline ? searched.npcKills : ProfileSearch.npcKills, "BOSS"), 25745);
				player.getPA().sendFrame126("Profile of " + Misc.capitalize(player.getProfileNameSearched()), 25325);
				if (!FileUtility.accountExists("backup/logs/rare drop log/" + player.getProfileNameSearched() + ".txt"))
				{
		...
		{
				Player searched = PlayerHandler.players[player.getProfileSearchOnlinePlayerId()];
				long totalXp = (ProfileSearch.getExperienceTotal(player.isProfileSearchOnline ? searched.skillExperience : ProfileSearch.skillExperience, player.isProfileSearchOnline ? searched.getAbleToEditCombat() : ProfileSearch.ableToEditCombat, player.isProfileSearchOnline ? searched.combatExperienceGainedAfterMaxed : ProfileSearch.combatExperienceGainedAfterMaxed) / 1000000);
				String[] text = {
						"Total experience: " + totalXp + "m",
						"Highest: " + ProfileSearch.getHighestSkillXp(player.isProfileSearchOnline ? searched.getAbleToEditCombat() : ProfileSearch.ableToEditCombat, player.isProfileSearchOnline ? searched.skillExperience : ProfileSearch.skillExperience, player.isProfileSearchOnline ? searched.combatExperienceGainedAfterMaxed : ProfileSearch.combatExperienceGainedAfterMaxed),
						"Bones buried: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.BONES_BURIED] : ProfileSearch.skillingStatistics[SkillingStatistics.BONES_BURIED]),
						"Bones on altar: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.BONES_ON_ALTAR] : ProfileSearch.skillingStatistics[SkillingStatistics.BONES_ON_ALTAR]),
						"High alchemy: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.HIGH_ALCHEMY] : ProfileSearch.skillingStatistics[SkillingStatistics.HIGH_ALCHEMY]),
						"Runes crafted: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.RUNE_ESSENCE_CRAFTED] : ProfileSearch.skillingStatistics[SkillingStatistics.RUNE_ESSENCE_CRAFTED]),
						"Agility laps: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.AGILITY_LAPS] : ProfileSearch.skillingStatistics[SkillingStatistics.AGILITY_LAPS]),
						"Potions made: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.POTIONS_MADE] : ProfileSearch.skillingStatistics[SkillingStatistics.POTIONS_MADE]),
						"Stalls thieved: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.STALLS_THIEVED] : ProfileSearch.skillingStatistics[SkillingStatistics.STALLS_THIEVED]),
						"Pickpockets: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.PICKPOCKETS] : ProfileSearch.skillingStatistics[SkillingStatistics.PICKPOCKETS]),
						"Gems crafted: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.GEMS_CRAFTED] : ProfileSearch.skillingStatistics[SkillingStatistics.GEMS_CRAFTED]),
						"Leather crafted: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.LEATHER_CRAFTED] : ProfileSearch.skillingStatistics[SkillingStatistics.LEATHER_CRAFTED]),
						"Bows strung: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.BOWS_MADE] : ProfileSearch.skillingStatistics[SkillingStatistics.BOWS_MADE]),
						"Slayer tasks: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.SLAYER_TASKS] : ProfileSearch.skillingStatistics[SkillingStatistics.SLAYER_TASKS]),
						"Ores mined: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.ORES_MINED] : ProfileSearch.skillingStatistics[SkillingStatistics.ORES_MINED]),
						"Bars smelted: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.BARS_SMELTED] : ProfileSearch.skillingStatistics[SkillingStatistics.BARS_SMELTED]),
						"Items smithed: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.ITEMS_SMITHED] : ProfileSearch.skillingStatistics[SkillingStatistics.ITEMS_SMITHED]),
						"Fish caught: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.FISH_CAUGHT] : ProfileSearch.skillingStatistics[SkillingStatistics.FISH_CAUGHT]),
						"Fish cooked: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.FISH_COOKED] : ProfileSearch.skillingStatistics[SkillingStatistics.FISH_COOKED]),
						"Logs burnt: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.LOGS_BURNT] : ProfileSearch.skillingStatistics[SkillingStatistics.LOGS_BURNT]),
						"Logs from trees: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.LOGS_GAINED] : ProfileSearch.skillingStatistics[SkillingStatistics.LOGS_GAINED]),
						"Seeds planted: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.SEEDS_PLANTED] : ProfileSearch.skillingStatistics[SkillingStatistics.SEEDS_PLANTED]),
						"Jewelry created: " + (player.isProfileSearchOnline ? searched.skillingStatistics[SkillingStatistics.JEWELRY_CREATED] : ProfileSearch.skillingStatistics[SkillingStatistics.JEWELRY_CREATED]),};
				...
				for (int i = 0; i < SKILL_TAB_SKILL_ID_ORDER.length; i++)
				{
						...
						{
								player.getPA().sendFrame126("Xp: " + (13034431 + (player.isProfileSearchOnline ? searched.combatExperienceGainedAfterMaxed[SKILL_TAB_SKILL_ID_ORDER[i]] : ProfileSearch.combatExperienceGainedAfterMaxed[SKILL_TAB_SKILL_ID_ORDER[i]])), (25602 + interfaceIdIndex));
						}
						else
						{
								player.getPA().sendFrame126("Xp: " + (player.isProfileSearchOnline ? searched.skillExperience[SKILL_TAB_SKILL_ID_ORDER[i]] : ProfileSearch.skillExperience[SKILL_TAB_SKILL_ID_ORDER[i]]), (25602 + interfaceIdIndex));
						}
						interfaceIdIndex += 4;
				}
				...
				for (int i = 0; i < SKILL_TAB_SKILL_ID_ORDER.length; i++)
				{
						...
						{
								player.getPA().sendFrame126((player.isProfileSearchOnline ? searched.baseSkillLevel[SKILL_TAB_SKILL_ID_ORDER[i]] : ProfileSearch.skillLevel[SKILL_TAB_SKILL_ID_ORDER[i]]) + "", (25603 + interfaceIdIndex));
						}
						interfaceIdIndex += 4;
				}
		...
		public static void viewMiscPage(Player player)
		{
				...
				String[] text = {
						...
						"Crystal keys used: " + Achievements.getArraylistCount("crystalKeysUsed", player.isProfileSearchOnline ? searched.achievementProgress : ProfileSearch.achievementProgress),
						"",
						"",
						"",
						"",
		...
		public static void viewPlayerKillingPage(Player player)
		{
				...
				if (player.isProfileSearchOnline)
				{
						ProfileSearch.wildernessKills = searched.wildernessKills;
						ProfileSearch.wildernessDeaths = searched.wildernessDeaths;
						...
						ProfileSearch.skillingStatistics = searched.skillingStatistics;
						ProfileSearch.killsInMulti = searched.killsInMulti;
				}
				double kdr = QuestTab.getKDR(ProfileSearch.wildernessKills, ProfileSearch.wildernessDeaths);
				double kdr1 = QuestTab.getKDR(ProfileSearch.meleeMainKills, player.isProfileSearchOnline ? searched.deathTypes[AchievementStatistics.MELEE_MAIN] : ProfileSearch.deathTypes[AchievementStatistics.MELEE_MAIN]);
				double kdr2 = QuestTab.getKDR(ProfileSearch.hybridKills, player.isProfileSearchOnline ? searched.deathTypes[AchievementStatistics.HYBRID] : ProfileSearch.deathTypes[AchievementStatistics.HYBRID]);
				double kdr3 = QuestTab.getKDR(ProfileSearch.berserkerPureKills, player.isProfileSearchOnline ? searched.deathTypes[AchievementStatistics.BERSERKER] : ProfileSearch.deathTypes[AchievementStatistics.BERSERKER]);
				double kdr4 = QuestTab.getKDR(ProfileSearch.pureKills, player.isProfileSearchOnline ? searched.deathTypes[AchievementStatistics.PURE] : ProfileSearch.deathTypes[AchievementStatistics.PURE]);
				double kdr5 = QuestTab.getKDR(ProfileSearch.rangedTankKills, player.isProfileSearchOnline ? searched.deathTypes[AchievementStatistics.RANGED_TANK] : ProfileSearch.deathTypes[AchievementStatistics.RANGED_TANK]);
				String[] text = {
						"Kills: " + ProfileSearch.wildernessKills + ", Deaths: " + ProfileSearch.wildernessDeaths + ", Ratio: " + kdr,
						"@yel@Melee main",
						ProfileSearch.meleeMainKills + " | " + (player.isProfileSearchOnline ? searched.deathTypes[AchievementStatistics.MELEE_MAIN] : ProfileSearch.deathTypes[AchievementStatistics.MELEE_MAIN]) + " | " + kdr1,
						"@yel@Hybrid",
						ProfileSearch.hybridKills + " | " + (player.isProfileSearchOnline ? searched.deathTypes[AchievementStatistics.HYBRID] : ProfileSearch.deathTypes[AchievementStatistics.HYBRID]) + " | " + kdr2,
						"@yel@Berserker pure",
						ProfileSearch.berserkerPureKills + " | " + (player.isProfileSearchOnline ? searched.deathTypes[AchievementStatistics.BERSERKER] : ProfileSearch.deathTypes[AchievementStatistics.BERSERKER]) + " | " + kdr3,
						"@yel@Pure",
						ProfileSearch.pureKills + " | " + (player.isProfileSearchOnline ? searched.deathTypes[AchievementStatistics.PURE] : ProfileSearch.deathTypes[AchievementStatistics.PURE]) + " | " + kdr4,
						"@yel@Ranged tank",
						ProfileSearch.rangedTankKills + " | " + (player.isProfileSearchOnline ? searched.deathTypes[AchievementStatistics.RANGED_TANK] : ProfileSearch.deathTypes[AchievementStatistics.RANGED_TANK]) + " | " + kdr5,
