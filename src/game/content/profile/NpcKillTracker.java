...
public class NpcKillTracker
{
		...
		/**
		 * Boss list to track, including Jad.
		 * Modify BossPetDrops array too.
		 */
		public final static int[] BOSS_LIST = {2881, 2882, 2883, 4040, 6203, 6247, 6222, 6260, 8133, 3200, 50, 2745, 4043, 8349, 4045, 9463};
		...
		private static void sendBossKillCountMessage(Player player, String npcName, int npcId)
		{
				...
				{
						if (npcName.equals(NpcDefinition.getDefinitions()[BossPetDrops.NORMAL_BOSS_DATA[i][0]].name))
						{
								sendMessage = true;
								break;
						}
				...
				if (sendMessage)
				{
						if (HighscoresDaily.getInstance().currentDailyHighscores.equals("Boss kills"))
						{
								HighscoresDaily.getInstance().sortHighscores(player, 1, NpcKillTracker.getNpcAmount(player.npcKills, "BOSS"), NpcKillTracker.getHighestBossKilled(player));
						}
		...
		*/
		...
		/**
		 * @return
		 * 			The highest boss name and kill count.
		 */
		public static String getHighestBossKilled(Player player)
		{
				...
				for (int index = 0; index < player.npcKills.size(); index++)
				{
						...
						{
								if (npcName.equals(NpcDefinition.getDefinitions()[BOSS_LIST[i]].name))
								{
		 ...
		 * @param capped
		 * 		True, to limit the elo per boss. This is only true when calculating Adventurer rank which is 100% needed.
		 */
		...
		/**
		 * @param npcType
		 * 			To count the total kills of the given type.
		 * @return
		 * 			The npc amount of the npcType given.
		 */
