...
public class ProfileRank
{
		...
		public static void rankPopUp(Player player, String rankType)
		{
				...
				{
						case "ADVENTURER":
								int updatedRank = getAdventurerRankNumber(player.getTotalLevel(), player.bossScoreCapped, player.getBarrowsRunCompleted(), player.getClueScrollsCompleted(), player.titleTotal, player.achievementTotal);
						...
						case "PKER":
								int updatedRank1 = getPkingRankNumber(player.getMeleeMainKills(), player.getHybridKills(), player.getPureKills(), player.getBerserkerPureKills(), player.getRangedTankKills());
		...
		{
				player.currentAdventurerRank = getAdventurerRankNumber(player.getTotalLevel(), player.bossScoreCapped, player.getBarrowsRunCompleted(), player.getClueScrollsCompleted(), player.titleTotal, player.achievementTotal);
				player.currentPkerRank = getPkingRankNumber(player.getMeleeMainKills(), player.getHybridKills(), player.getPureKills(), player.getBerserkerPureKills(), player.getRangedTankKills());
		}
		...
		public static void applyRank(Player player, String rankType)
		{
				...
				if (rankType.equals("PLAYER KILLING"))
				{
						...
						int rank = getPkingRankNumber(player.isProfileSearchOnline ? searched.getMeleeMainKills() : ProfileSearch.meleeMainKills, player.isProfileSearchOnline ? searched.getHybridKills() : ProfileSearch.hybridKills, player.isProfileSearchOnline ? searched.getPureKills() : ProfileSearch.pureKills, player.isProfileSearchOnline ? searched.getBerserkerPureKills() : ProfileSearch.berserkerPureKills, player.isProfileSearchOnline ? searched.getRangedTankKills() : ProfileSearch.rangedTankKills);
						rankName = ranks[rank];
				...
				{
						int rank = getAdventurerRankNumber(player.isProfileSearchOnline ? searched.getTotalLevel() : ProfileSearch.totalLevel, player.isProfileSearchOnline ? searched.bossScoreCapped : ProfileSearch.bossScoreCapped, player.isProfileSearchOnline ? searched.getBarrowsRunCompleted() : ProfileSearch.barrowsRunCompleted, player.isProfileSearchOnline ? searched.getClueScrollsCompleted() : ProfileSearch.clueScrollsCompleted, player.isProfileSearchOnline ? searched.titleTotal : ProfileSearch.titleTotal, player.isProfileSearchOnline ? searched.achievementTotal : ProfileSearch.achievementTotal);
						rankName = ranks[rank];
