...
public class Zombie
{
		...
		private static void startMinigame(Player playerOne, Player playerTwo)
		{
				...
				QuickSetUp.heal(playerOne);
				QuickSetUp.heal(playerTwo);
				...
				ZombieGameInstance instance = new ZombieGameInstance(playerOne.getPlayerName(), playerTwo.getPlayerName(), 1, ZombieWaveInstance.getWaveData(1).getChestCoordinates(), getZombiesToSpawn(1, getZombiesTotalForWave(1)), ZombieWaveInstance.getWaveData(1).getChestId(), playerOne.getPlayerId() * (0 + 4), getZombiesTotalForWave(1), ZombieWaveInstance.getWaveData(1).zombieSpawns, true, true);
		...
		public static void loadChest(Player player)
		{
				...
				player.getPA().spawnClientObject(ZombieGameInstance.instance.get(minigameInstanceIndex).getChestId(), ZombieGameInstance.instance.get(minigameInstanceIndex).getChestXCoordinate(), ZombieGameInstance.instance.get(minigameInstanceIndex).getChestYCoordinate(), ZombieGameInstance.instance.get(minigameInstanceIndex).getChestFace(), 10);
		...
		private static void newWave(final Player playerOne, final Player playerTwo)
		{
				...
				{
						teleported = true;
						Teleport.startTeleport(playerOne, ZombieWaveInstance.instance.get(waveDataIndex).getTeleportXCoordinate() + Misc.random(3), ZombieWaveInstance.instance.get(waveDataIndex).getTeleportYCoordinate() + Misc.random(3), ZombieGameInstance.instance.get(minigameInstanceIndex).getHeight(), "ZOMBIE");
						if (playerTwo != null)
						{
								Teleport.startTeleport(playerTwo, ZombieWaveInstance.instance.get(waveDataIndex).getTeleportXCoordinate() + Misc.random(3), ZombieWaveInstance.instance.get(waveDataIndex).getTeleportYCoordinate() + Misc.random(3), ZombieGameInstance.instance.get(minigameInstanceIndex).getHeight(), "ZOMBIE");
						}
		...
		private static void spawnBoss(Player player)
		{
				...
				NpcHandler.spawnNpcZombie(player, ZombieWaveInstance.getWaveData(ZombieGameInstance.instance.get(minigameInstanceIndex).wave).bossId, Integer.parseInt(coordinateRandom[0]), Integer.parseInt(coordinateRandom[1]), ZombieGameInstance.instance.get(minigameInstanceIndex).getHeight());
		}
		...
		public static void spawnZombies(Player playerOne, Player playerTwo)
		{
				...
				for (int index = 0; index < zombiesToSpawn; index++)
				{
						...
						String[] coordinateRandom = zombieSpawns.get(Misc.random(zombieSpawns.size() - 1)).split(" ");
						NpcHandler.spawnNpcZombie(spawnForPlayerOne ? playerOne : playerTwo, npcId, Integer.parseInt(coordinateRandom[0]), Integer.parseInt(coordinateRandom[1]), ZombieGameInstance.instance.get(minigameInstanceIndex).getHeight());
						list.remove(npcId + "");
				}
				String zombieList = "";
				if (!noneLeft)
		...
		*/
		public static boolean zombieKilled(Player player, Npc npc)
		{
				...
				waveSpecificDrops(player, ZombieGameInstance.instance.get(minigameInstanceIndex).totalWaveZombiesLeft, ZombieGameInstance.instance.get(minigameInstanceIndex).wave, minigameInstanceIndex, npc);
		...
		private static void starterKit(Player player)
		{
				...
				// Toolkit.
				Server.itemHandler.createGroundItem(player, 4051, player.getX() + Misc.random(2), player.getY() + Misc.random(2), 1, true, 0, true, "");
				...
				for (int index = 0; index < list.size(); index++)
				{
						Server.itemHandler.createGroundItem(player, list.get(index), player.getX() + Misc.random(2), player.getY() + Misc.random(2), 1, true, 0, true, "");
						Server.itemHandler.createGroundItem(player, 315, player.getX() + Misc.random(2), player.getY() + Misc.random(2), 1, true, 0, true, "");
				}
		...
		private static void bossDrops(Player player, Npc npc)
		{
				...
				Server.itemHandler.createGroundItem(player, itemId, npc.getVisualX(), npc.getVisualY(), amount, true, 0, true, "OTHER: empty");
		}
		...
		*/
						...
						for (int i = 0; i < list.size(); i++)
						{
								...
								if (itemIdRequested == itemIdStock)
								{
										...
										if (points >= price)
										{
												...
												else if (ItemDefinition.getDefinitions()[itemIdRequested].stackable && !ItemAssistant.hasItemInInventory(player, itemIdRequested) && ItemAssistant.getFreeInventorySlots(player) == 0)
												{
														amountRequested = 0;
												}
										...
										{
												player.playerAssistant.sendMessage(ItemAssistant.getItemName(itemIdRequested) + ": " + (price - points) + " additional Zombie points are required to purchase this item.");
										}
										break;
								}
						}
		...
		private static void deleteMinigameInstance(int minigameInstanceIndex, Player player)
		{
				...
				for (int i = 0; i < NpcHandler.NPC_INDEX_OPEN_MAXIMUM; i++)
				{
						...
						if (npc.zombieOwner.equals(ZombieGameInstance.instance.get(minigameInstanceIndex).getPlayerOneName()) || npc.zombieOwner.equals(ZombieGameInstance.instance.get(minigameInstanceIndex).getPlayerTwoName()))
						{
								Pet.deletePet(npc);
						}
				}
		...
		*/
		...
		private static void dropWaveItem(Player player, int minigameInstance, Npc npc)
		{
				...
				{
						item = NpcDrops.mysticGloves[Misc.random(NpcDrops.mysticGloves.length - 1)];
				}

				// Mystic hat
				else if (item == 4089)
				{
						item = NpcDrops.mysticHats[Misc.random(NpcDrops.mysticHats.length - 1)];
				}

				// Mystic top
				else if (item == 4091)
				{
						item = NpcDrops.mysticTops[Misc.random(NpcDrops.mysticTops.length - 1)];
				}

				// Mystic bottom
				else if (item == 4093)
				{
						item = NpcDrops.mysticBottoms[Misc.random(NpcDrops.mysticBottoms.length - 1)];
				}

				// Mystic boots
				else if (item == 4097)
				{
						item = NpcDrops.mysticBoots[Misc.random(NpcDrops.mysticBoots.length - 1)];
				}
				Server.itemHandler.createGroundItem(player, item, npc.getVisualX(), npc.getVisualY(), 1, true, 0, true, "");
		}
}
