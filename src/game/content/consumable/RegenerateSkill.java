...
public class RegenerateSkill
{
				...
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent()
				{
						...
						public void execute(CycleEventContainer container)
						{
								...
								for (int value = 0; value <= 6; value++)
								{
										...
										else if (player.currentCombatSkillLevel[value] < player.baseSkillLevel[value])
										{
		...
		/**
		 * @param boostedTime
		 * 			The time of when the skill was boosted.
		 * @return
		 * 			True, if the given boostedTime has occured over 60 seconds ago.
		 */
		private static boolean hasMinuteElapsed(long boostedTime, boolean preservePrayer)
		{
				int amount = preservePrayer ? 72000 : 60000;
