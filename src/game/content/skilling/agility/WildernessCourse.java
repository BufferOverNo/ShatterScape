...
public class WildernessCourse
{
		...
		public final static int[][] wildernessCourseExperience = {{3004, 3950, 13
						// Obstacle pipe.
						}, {3005, 3958, 20
						// Rope swing.
						}, {2997, 3960, 20
		...
		private static int[] wildernessCourseObjects = {DOOR_ENTRANCE, OBSTACLE_PIPE, ROPE_SWING, STEPPING_STONE, LOG_BALANCE, ROCKS, DOOR_EXIT, DOOR_EXIT_NEXT};
		...
		*/
		...
		private static void rocksAction(Player player)
		{
				...
				{
						Skilling.addSkillExperience(player, 499, ServerConstants.AGILITY);
						int chance = 0;
						int amount = 4;
						if (GameMode.getGameMode(player, "GLADIATOR") || GameMode.getGameMode(player, "IRON MAN"))
						{
								amount += 1;
						}
						amount = amount * (Misc.hasPercentageChance(chance) ? 2 : 1);
						Skilling.addSkillExperience(player, 47, ServerConstants.AGILITY);
						player.agilityPoints += amount;
						String s = amount == 1 ? "" : "s";
						player.playerAssistant.sendFilterableMessage("The wizards award you " + amount + " agility point" + s + " for your efforts.");
