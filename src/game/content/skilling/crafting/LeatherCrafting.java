...
public class LeatherCrafting
{
		...
		public static void showCraftingInterface(Player player, String title, int leftImageItem, int middleImageItem, int rightImageItem, String leftImageName, String middleImageName, String rightImageName, int zoom)
		{
		...
		public static void hardLeatherBodyInterfaceAction(Player player, int amount)
		{
				...
				if (amount == 100)
				{
						player.setAmountInterface("HARD LEATHER BODY");
		...
		private static void craftLeather(Player player, int amount, boolean skipXCheck)
		{
				// Send x amount interface.
				if (amount == 27 && !skipXCheck)
				{
						player.setAmountInterface(ServerConstants.SKILL_NAME[ServerConstants.CRAFTING] + " LEATHER");
						player.getOutStream().createFrame(27);
						return;
				}
				player.skillingData[3] = amount;
		...
		private static void craftLeatherEvent(final Player player, final LeatherCraftingData leather)
		{
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent()
				{
						...
						public void execute(CycleEventContainer container)
						{
								...
								{
										player.getDH().sendStatement("You need " + leather.getRequiredAmount() + " " + ItemAssistant.getItemName(leather.getLeatherRequired()) + " to craft a " + ItemAssistant.getItemName(leather.getItemProduced()) + ".");
										container.stop();
										return;
								}
		...
		private static void successfulLeather(Player player, LeatherCraftingData leather)
		{
				...
				Skilling.addSkillExperience(player, (int) leather.getExperience(), ServerConstants.CRAFTING);
		...
		public static boolean isLeatherCraftingButton(Player player, int buttonId)
		{
				...
				for (int index = 0; index < leatherInterfaceButtons.length; index++)
				{
						if (buttonId == leatherInterfaceButtons[index][0])
						{
								...
								craftLeather(player, leatherInterfaceButtons[index][2], false);
								return true;
						}
				}
				...
				for (int index = 0; index < hideData.length; index++)
				{
						...
						if (engage)
						{
								...
								if (amount == 100)
								{
										...
										player.setAmountInterface("TANNING");
										player.getOutStream().createFrame(27);
										return true;
								}
