...
public class JewelryCrafting
{
		...
		private static void mouldJewelry(final Player player, int jewelryIndex)
		{
				...
				{
						player.getPA().sendMessage("You need " + ((itemRequired.startsWith("A") || itemRequired.startsWith("E") || itemRequired.startsWith("O")) ? "an" : "a") + " " + itemRequired.toLowerCase() + " to mould this item.");
						return;
				}
				...
				if (Skilling.canActivateNewSkillingEvent(player, "MOULDING JEWELRY"))
				{
						return;
				}
				...
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent()
				{
						...
						{
								if (!Skilling.isSkillingEventActive(player, "MOULDING JEWELRY"))
								{
										container.stop();
										return;
								}
								if ((ItemAssistant.hasItemInInventory(player, 2357) || player.isInZombiesMinigame()) && ItemAssistant.hasItemInInventory(player, required))
								{
										...
										Skilling.addSkillExperience(player, xp, ServerConstants.CRAFTING);
										player.skillingStatistics[SkillingStatistics.JEWELRY_CREATED]++;
										player.getPA().sendFilterableMessage("You make " + ((itemName.startsWith("A") || itemName.startsWith("E") || itemName.startsWith("O")) ? "an" : "a") + " " + itemName.toLowerCase());
								}
								else
								{
										container.stop();
		...
		public static void stringAmuletAmount(Player player, int amount)
		{
				...
				if (amount == 100)
				{
						player.setAmountInterface("STRINGING AMULET");
		...
		{
				if (Skilling.canActivateNewSkillingEvent(player, "STRINGING AMULET"))
				{
						return;
				}
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent()
				{
						...
						public void execute(CycleEventContainer container)
						{
								if (!Skilling.isSkillingEventActive(player, "STRINGING AMULET"))
								{
										container.stop();
										return;
								}
								...
								Skilling.addSkillExperience(player, 4, ServerConstants.CRAFTING);
