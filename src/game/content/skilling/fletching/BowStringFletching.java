...
public class BowStringFletching
{
		...
		public static void fletchingInterfaceAction(Player player, int amount)
		{
				...
				if (amount == 100)
				{
						player.setAmountInterface(ServerConstants.SKILL_NAME[ServerConstants.FLETCHING]);
		...
		{
				if (player.bowStringEvent)
				{
						return;
				}
				player.bowStringEvent = true;
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent()
				{
						...
						public void execute(CycleEventContainer container)
						{
								...
								Skilling.addSkillExperience(player, experience, ServerConstants.FLETCHING);
