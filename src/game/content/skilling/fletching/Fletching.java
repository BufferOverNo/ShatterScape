...
public class Fletching
{
		...
		private enum GemBolts
		{
				...
				private GemBolts(int gemToCut, int boltsToAttachTo, int fletchLevelRequired, int stoneCutXp, int xpPerTipAttachedToBolt, int cuttingGemAnimation, int tipsProducedId, int amountOfTipsPerGem, int boltToProduce)
				{
		...
		{
				if (Skilling.canActivateNewSkillingEvent(player, "ATTACH TIPS TO BOLT"))
				{
						return;
				}
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent()
				{
						...
						public void execute(CycleEventContainer container)
						{
								...
								if (!Skilling.isSkillingEventActive(player, "ATTACH TIPS TO BOLT"))
								{
										container.stop();
										return;
								}
								...
								{
										player.getDH().sendStatement("You need at least " + player.skillingData[2] + " to fletch " + ItemAssistant.getItemName(player.skillingData[4]) + ".");
										container.stop();
										return;
								}
								int amount = 15;
								...
								Skilling.addSkillExperience(player, player.skillingData[3] * amount, ServerConstants.FLETCHING);
						}
		...
		public static void attachTipToBoltAmount(Player player, int amount)
		{
				...
				if (amount == 100)
				{
						player.setAmountInterface("ATTACH TIPS TO BOLT");
		...
		{
				if (Skilling.canActivateNewSkillingEvent(player, "CUT GEM INTO BOLT TIPS"))
				{
						return;
				}
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent()
				{
						...
						public void execute(CycleEventContainer container)
						{
								...
								if (!Skilling.isSkillingEventActive(player, "CUT GEM INTO BOLT TIPS"))
								{
										container.stop();
										return;
								}
								...
								Skilling.addSkillExperience(player, player.skillingData[2], ServerConstants.FLETCHING);
		...
		public static void cutGemAmount(Player player, int amount)
		{
				...
				if (amount == 100)
				{
						player.setAmountInterface("CUT GEM INTO BOLT TIPS");
						player.getOutStream().createFrame(27);
						return;
				}
		...
		private static void combineArrowPartsEvent(final Player player)
		{
				...
				if (Skilling.canActivateNewSkillingEvent(player, "COMBINE ARROWS"))
				{
						return;
				}
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent()
				{
						...
						public void execute(CycleEventContainer container)
						{
								...
								if (!Skilling.isSkillingEventActive(player, "COMBINE ARROWS"))
								{
										container.stop();
										return;
								}
								...
								Skilling.addSkillExperience(player, player.skillingData[4], ServerConstants.FLETCHING);
						}
		...
		public static void combineArrowPartsAmount(Player player, int amount)
		{
				...
				if (amount == 100)
				{
						player.setAmountInterface("COMBINE ARROWS");
						player.getOutStream().createFrame(27);
						return;
				}
		...
		public static boolean others(Player player, int itemUsed, int useWith)
		{
				...
				for (int i = 0; i < data.length; i++)
				{
						if (((itemUsed == 946 || itemUsed == 4051) && useWith == data[i][0]) || (itemUsed == data[i][0] && (useWith == 946 || useWith == 4051)))
						{
								...
								{
										player.yewLog = true;
										LeatherCrafting.showCraftingInterface(player, "What would you like to make?", 68, 9452, 66, ItemAssistant.getItemName(9452), ItemAssistant.getItemName(68), ItemAssistant.getItemName(66), 200);
										return true;
								}
								showInterfaceOthers(player, new int[] {data[i][1], data[i][4],}, data[i][7]);
								return true;
						}
				}
				return false;
		...
		public static void attemptData(Player c, int amount, int type)
		{
				c.playerSkillProp[9][0] = normal[0][0]; //LOG
				switch (type)
				{
						case 0: //LONGBOW
								c.playerSkillProp[9][1] = 48; // Item
								c.playerSkillProp[9][2] = 10; // Level
								c.playerSkillProp[9][3] = 10; // Xp
								break;
						case 1: //SHORTBOW
								c.playerSkillProp[9][1] = 50;
		...
		private static void showInterfaceFletching(Player player, int[] items, int type)
		{
				...
				player.playerFletch = true;
		}

		public static int[][] normal = {{1511, 50, 48, 50, 1, 0, 6684, 52},};
		...
		public static void attemptData(final Player player, int amount, boolean second)
		{
				...
				player.rawBeefChosen = false;
				if (!Skilling.hasRequiredLevel(player, 9, player.playerSkillProp[9][2], "fletching", "fletch this"))
				{
						return;
				}

				if (Skilling.canActivateNewSkillingEvent(player, "FLETCHING"))
				{
						return;
				}
				...
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent()
				{
						...
						public void execute(CycleEventContainer container)
						{
								if (!Skilling.isSkillingEventActive(player, "FLETCHING"))
								{
										container.stop();
										return;
								}
								if (player.playerSkillProp[9][0] > 0)
								{
										...
										Skilling.addSkillExperience(player, addXP, 9);
		...
		public static void yewStockInterfaceAction(final Player player, int imageType, int amount)
		{
				...
				player.getPA().closeInterfaces();
				if (Skilling.canActivateNewSkillingEvent(player, "YEW STOCK"))
				{
						return;
				}
				...
				CycleEventHandler.getSingleton().addEvent(player, new CycleEvent()
				{
						...
						public void execute(CycleEventContainer container)
						{
								if (!Skilling.isSkillingEventActive(player, "YEW STOCK"))
								{
										container.stop();
										return;
								}
								...
								Skilling.addSkillExperience(player, xp1, ServerConstants.FLETCHING);
								player.startAnimation(6702);
								player.doAmount--;
						}
