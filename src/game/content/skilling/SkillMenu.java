...
public class SkillMenu
{
		...
		private static final String[][] DESCRIPTION = {
				{"Bronze Weapons", "Iron Weapons", "Steel Weapons", "Black Weapons", "Mithril Weapons", "Adamant Weapons", "Rune Weapons", "Granite Maul", "Dragon Weapons", "Abyssal Whip", "Barrows Weapons", "Godswords", "Cape of Achievement"},
				{
						"Bronze Armour",
						"Iron Armour",
						"Steel Armour",
		...
		public static void openInterface(Player c, int skillType)
		{
				...
				c.getPA().displayInterface(INTERFACE_ID);
		}
		...
		private static void writeItems(Player c, int skillType)
		{
				...
				c.getOutStream().writeWord(8847);
				c.getOutStream().writeWord(items[skillType].length);
				for (int j = 0; j < items[skillType].length; j++)
				{
						c.getOutStream().writeByte(1);
						if (items[skillType][j] > 0)
						{
								c.getOutStream().writeWordBigEndianA(items[skillType][j] + 1);
						}
						else
						{
