...
public class HighscoresPker
{
		...
		public void sortHighscores(Player player)
		{
				...
				int rank = ProfileRank.getPkingRankNumber(player.getMeleeMainKills(), player.getHybridKills(), player.getPureKills(), player.getBerserkerPureKills(), player.getRangedTankKills());
				double kdr = QuestTab.getKDR(player.wildernessKills, player.wildernessDeaths);
				for (int index = 0; index < highscoresList.length; index++)
				{
						...
						if (rank > highscoresList[index].rankNumber && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()) || player.wildernessKills > highscoresList[index].kill && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()) || kdr != highscoresList[index].kdr && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()))
						{
								highscoresList[index].rankNumber = rank;
								highscoresList[index].kill = player.wildernessKills;
						...
						else if (!isOnHighscores(player.getPlayerName()))
						{
								...
								{
										highscoresList[0] = new HighscoresPker(player.getPlayerName(), rank, player.wildernessKills, kdr, player.gameMode);
										break;
								}
						}
