...
public class HighscoresBarrowsRun
{
		...
		public HighscoresBarrowsRun(String username, int barrowsTime, int barrowsRunAmount, long bossScore, String gameMode)
		{
				this.name = username;
		...
		public void loadFile()
		{
				...
				try (FileReader fileReader = new FileReader(file))
				{
						...
						for (int i = 0; i < ServerConstants.HIGHSCORES_PLAYERS_AMOUNT; i++)
						{
								...
								{
										time = reader.get("rank " + i + ", time").getAsInt();
								}
		...
		public void sortHighscores(Player player)
		{
				...
				for (int index = 0; index < highscoresList.length; index++)
				{
						...
						if (player.barrowsPersonalRecord < (highscoresList[index].barrowsTime == 0 ? Integer.MAX_VALUE : highscoresList[index].barrowsTime) && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()) || player.getBarrowsRunCompleted() > highscoresList[index].barrowsRunAmount && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()))
						{
						...
						else if (!isOnHighscores(player.getPlayerName()))
						{
								...
								{
										highscoresList[0] = new HighscoresBarrowsRun(player.getPlayerName(), player.barrowsPersonalRecord, player.getBarrowsRunCompleted(), System.currentTimeMillis(), player.gameMode);
										break;
								}
						}
				...
				for (int counter = 0; counter < highscoresList.length - 1; counter++)
				{
						...
						{
								if ((highscoresList[index].barrowsTime == 0 ? Integer.MAX_VALUE : highscoresList[index].barrowsTime) < (highscoresList[index + 1].barrowsTime == 0 ? Integer.MAX_VALUE : highscoresList[index + 1].barrowsTime))
								{
