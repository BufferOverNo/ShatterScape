...
public class HighscoresAdventurer
{
		...
		public void sortHighscores(Player player)
		{
				...
				int rank = ProfileRank.getAdventurerRankNumber(player.getTotalLevel(), player.bossScoreCapped, player.getBarrowsRunCompleted(), player.getClueScrollsCompleted(), player.titleTotal, player.achievementTotal);
				...
				for (int index = 0; index < highscoresList.length; index++)
				{
						...
						if (rank > highscoresList[index].rankNumber && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()) || player.getTotalLevel() > highscoresList[index].totalLevel && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()) || totalBossKills > highscoresList[index].bossKill && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()))
						{
						...
						else if (!isOnHighscores(player.getPlayerName()))
						{
								...
								{
										highscoresList[0] = new HighscoresAdventurer(player.getPlayerName(), rank, player.getTotalLevel(), totalBossKills, player.gameMode);
										break;
								}
						}
