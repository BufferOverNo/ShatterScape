 ...
 */
public class HighscoresTotalLevelVeteran
{
		/**
		 * The class instance.
		 */
		private static final HighscoresTotalLevelVeteran instance = new HighscoresTotalLevelVeteran();
		 ...
		 */
		public final static HighscoresTotalLevelVeteran getInstance()
		{
				return instance;
		}

		private final static String HIGHSCORES_LOCATION = "backup/logs/highscores/total level veteran.json";

		public HighscoresTotalLevelVeteran[] highscoresList;
		...
		public HighscoresTotalLevelVeteran()
		{
				this.name = "";
				this.totalLevel = 0;
				this.xp = 0;
		...
		public HighscoresTotalLevelVeteran(String username, int totalLevel, int xpInMillion, String highestSkill, String gameMode)
		{
				this.name = username;
		...
		{
				highscoresList = new HighscoresTotalLevelVeteran[ServerConstants.HIGHSCORES_PLAYERS_AMOUNT];
				for (int i = 0; i < ServerConstants.HIGHSCORES_PLAYERS_AMOUNT; i++)
				{
						highscoresList[i] = new HighscoresTotalLevelVeteran();
				}
				loadFile();
		}
		...
		public void loadFile()
		{
				...
				try (FileReader fileReader = new FileReader(file))
				{
						...
						for (int i = 0; i < ServerConstants.HIGHSCORES_PLAYERS_AMOUNT; i++)
						{
								...
								if (!name.isEmpty())
								{
										highscoresList[i] = new HighscoresTotalLevelVeteran(name, totalLevel, xp, highestSkill, gameMode);
								}
						}
				}
				catch (IOException e)
		...
		public void removePlayer(String name)
		{
				for (int index = 0; index < highscoresList.length; index++)
				{
						...
						{
								highscoresList[index] = new HighscoresTotalLevelVeteran("", 0, 0, "", "");
						}
				}
		}
		...
		public void sortHighscores(Player player)
		{
				...
				for (int index = 0; index < highscoresList.length; index++)
				{
						...
						else if (!isOnHighscores(player.getPlayerName()))
						{
								...
								{
										highscoresList[0] = new HighscoresTotalLevelVeteran(player.getPlayerName(), player.getTotalLevel(), (int) (player.getXpTotal() / 1000000), ProfileSearch.getHighestSkillXp(player.getAbleToEditCombat(), player.skillExperience, player.combatExperienceGainedAfterMaxed), player.gameMode);
										break;
								}
						}
				...
				for (int counter = 0; counter < highscoresList.length - 1; counter++)
				{
						for (int index = 0; index < highscoresList.length - 1 - counter; index++)
						{
								...
								{
										HighscoresTotalLevelVeteran temp = highscoresList[index];
