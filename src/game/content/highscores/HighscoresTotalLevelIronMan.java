 ...
 */
public class HighscoresTotalLevelIronMan
{
		/**
		 * The class instance.
		 */
		private static final HighscoresTotalLevelIronMan instance = new HighscoresTotalLevelIronMan();
		 ...
		 */
		public final static HighscoresTotalLevelIronMan getInstance()
		{
				return instance;
		}

		private final static String HIGHSCORES_LOCATION = "backup/logs/highscores/total level iron man.json";

		public HighscoresTotalLevelIronMan[] highscoresList;
		...
		public HighscoresTotalLevelIronMan()
		{
				this.name = "";
				this.totalLevel = 0;
				this.xp = 0;
		...
		public HighscoresTotalLevelIronMan(String username, int totalLevel, int xpInMillion, String highestSkill, String gameMode)
		{
				this.name = username;
		...
		{
				highscoresList = new HighscoresTotalLevelIronMan[ServerConstants.HIGHSCORES_PLAYERS_AMOUNT];
				for (int i = 0; i < ServerConstants.HIGHSCORES_PLAYERS_AMOUNT; i++)
				{
						highscoresList[i] = new HighscoresTotalLevelIronMan();
				}
				loadFile();
		}
		...
		public void loadFile()
		{
				...
				try (FileReader fileReader = new FileReader(file))
				{
						...
						for (int i = 0; i < ServerConstants.HIGHSCORES_PLAYERS_AMOUNT; i++)
						{
								...
								if (!name.isEmpty())
								{
										highscoresList[i] = new HighscoresTotalLevelIronMan(name, totalLevel, xp, highestSkill, gameMode);
								}
						}
				}
				catch (IOException e)
		...
		public void removePlayer(String name)
		{
				for (int index = 0; index < highscoresList.length; index++)
				{
						...
						{
								highscoresList[index] = new HighscoresTotalLevelIronMan("", 0, 0, "", "");
						}
				}
		}
		...
		public void sortHighscores(Player player)
		{
				...
				for (int index = 0; index < highscoresList.length; index++)
				{
						...
						if (player.getTotalLevel() > highscoresList[index].totalLevel && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()) || (player.getXpTotal() / 1000000) > highscoresList[index].xp && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()))
						{
						...
						else if (!isOnHighscores(player.getPlayerName()))
						{
								...
								{
										highscoresList[0] = new HighscoresTotalLevelIronMan(player.getPlayerName(), player.getTotalLevel(), (int) (player.getXpTotal() / 1000000), ProfileSearch.getHighestSkillXp(player.getAbleToEditCombat(), player.skillExperience, player.combatExperienceGainedAfterMaxed), player.gameMode);
										break;
								}
						}
				...
				for (int counter = 0; counter < highscoresList.length - 1; counter++)
				{
						for (int index = 0; index < highscoresList.length - 1 - counter; index++)
						{
								...
								{
										HighscoresTotalLevelIronMan temp = highscoresList[index];
