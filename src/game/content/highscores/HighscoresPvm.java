...
public class HighscoresPvm
{
		...
		public void sortHighscores(Player player)
		{
				...
				for (int index = 0; index < highscoresList.length; index++)
				{
						...
						if (bossKillScore >= highscoresList[index].score && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()) || totalBossKills > highscoresList[index].bossKills && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()))
						{
						...
						else if (!isOnHighscores(player.getPlayerName()))
						{
								...
								{
										highscoresList[0] = new HighscoresPvm(player.getPlayerName(), bossKillScore, totalBossKills, highestBossKilled, player.gameMode);
										break;
								}
						}
