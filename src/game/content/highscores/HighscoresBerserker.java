...
public class HighscoresBerserker
{
		...
		public void sortHighscores(Player player)
		{
				...
				for (int index = 0; index < highscoresList.length; index++)
				{
						...
						if (player.getBerserkerPureKills() > highscoresList[index].kills && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()) || player.deathTypes[AchievementStatistics.BERSERKER] > highscoresList[index].deaths && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()))
						{
						...
						else if (!isOnHighscores(player.getPlayerName()))
						{
								...
								{
										highscoresList[0] = new HighscoresBerserker(player.getPlayerName(), player.getBerserkerPureKills(), player.deathTypes[AchievementStatistics.BERSERKER], QuestTab.getKDR(player.getBerserkerPureKills(), player.deathTypes[AchievementStatistics.BERSERKER]), player.gameMode);
										break;
								}
						}
