...
public class HighscoresMelee
{
		...
		public void sortHighscores(Player player)
		{
				...
				for (int index = 0; index < highscoresList.length; index++)
				{
						...
						if (player.getMeleeMainKills() > highscoresList[index].kills && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()) || player.deathTypes[AchievementStatistics.MELEE_MAIN] > highscoresList[index].deaths && highscoresList[index].name.equalsIgnoreCase(player.getPlayerName()))
						{
						...
						else if (!isOnHighscores(player.getPlayerName()))
						{
								...
								{
										highscoresList[0] = new HighscoresMelee(player.getPlayerName(), player.getMeleeMainKills(), player.deathTypes[AchievementStatistics.MELEE_MAIN], QuestTab.getKDR(player.getMeleeMainKills(), player.deathTypes[AchievementStatistics.MELEE_MAIN]), player.gameMode);
										break;
								}
						}
