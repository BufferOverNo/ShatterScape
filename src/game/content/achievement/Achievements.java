...
public class Achievements
{
		...
		private final static int[] DIFFICULTY_BUTTONS = {86253, 87000, 87003, 87006};

		private final static int[] REWARDS_DIFFICULTY_BUTTONS = {75111, 75114, 75117, 75120};

		private final static String[] DIFFICULTY_NAMES = {"EASY", "MEDIUM", "HARD", "ELITE"};
		...
		private static void achievementReward(Player player, int achievementId, int achievementIndex)
		{
				...
				AchievementStatistics.showAchievementPopup(player, AchievementDefinitions.getDefinitions()[achievementIndex].descriptionSubText1, "Congratulations!", "You have unlocked new items.");
		...
		private static void updateAchievementDetails(Player player, int indexButton, boolean clicked)
		{
				...
				String[] items = AchievementDefinitions.getDefinitions()[indexButton].itemRewards.split(" ");
				for (int index = 0; index < 12; index++)
				{
						if (index >= items.length)
						{
								player.getPA().sendFrame34a(22293, 0, index, 1);
								continue;
						}
						player.getPA().sendFrame34a(22293, Integer.parseInt(items[index]), index, 1);
				}
		...
		{
				int completeAmount = AchievementDefinitions.getDefinitions()[indexButton].completeAmount;
				player.getPA().sendFrame126(currentProgress >= completeAmount ? "Complete!" : "Progress: " + Misc.formatRunescapeStyle(currentProgress) + "/" + Misc.formatRunescapeStyle(completeAmount), 22263);
		}
		...
		private static void displayAchievementInterface(Player player)
		{
		...
		public static boolean isAchievementButton(Player player, int buttonId)
		{
				...
				for (int index = 0; index < DIFFICULTY_BUTTONS.length; index++)
				{
						if (buttonId == DIFFICULTY_BUTTONS[index])
						{
								...
								player.getPA().setInterfaceClicked(22269 + (index * 3), true);
								displayScrollTitles(player);
								clearAchievementDetails(player);
								updateCompletedAmount(player);
				...
				for (int index = 0; index < REWARDS_DIFFICULTY_BUTTONS.length; index++)
				{
						if (buttonId == REWARDS_DIFFICULTY_BUTTONS[index])
						{
								...
								player.getPA().setInterfaceClicked(19311 + (index * 3), true);
								return true;
						}
				}
				return false;
		 ...
		 * @param currentAmount
		 * 			The current amount of Nomad kills/Barrows runs being assessed.
		 * @param minimumAmount
		 * 			The minimum value to show 0% completion.
		 * @param maximumAmount
		 * 			The maximum value to show 100% completion.
		 * @return
		 * 			The percentage of the current task/level completion.
		 */
		...
		public static int[] ACHIEVEMENT_KILLS_LIST = {10, 30, 50, 70, 100};
		...
		{
				int itemId = 18741;
				if (GameMode.getGameMode(player, "VETERAN") || GameMode.getGameMode(player, "DEFENDER"))
				{
						itemId = 18742;
				}
				...
				else if (player.achievementDifficultyCompleted[Achievements.HARD] && !player.achievementRewardClaimed[Achievements.HARD])
				{
						...
						ItemAssistant.addItem(player, 18680, 1);
