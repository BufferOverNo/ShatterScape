...
public class CoinCasket
{
		 ...
		 * @return
		 * 			True if the itemId given matches the Casket id.
		 */
		public static boolean isCoinCasketItemId(Player player, int itemId, int slot)
		{
				...
				switch (itemId)
				{
						...
						case 2714:
								amount = Misc.random(70, 120);
								break;

						case 2717:
								amount = Misc.random(40, 60);
								break;

						case 2720:
								amount = Misc.random(20, 35);
				...
				if (amount > 0)
				{
						...
						player.playerAssistant.sendMessage("You receive " + Misc.formatNumber((amount * 1000)) + " coins from the casket.");
						return true;
				}
				return false;
		}
		...
		public static void dropCoinCasket(Player player, Npc npc, int npcId, int npcHp, int x, int y)
		{
				...
				if (NpcDrops.isDropNothingNpc(npcId))
				{
						return;
				}
				for (int index = 0; index < NpcDrops.REVENANT_LIST.length; index++)
				{
						if (npcId == NpcDrops.REVENANT_LIST[index])
						{
								return;
						}
				}
				...
				int amount = Area.inWilderness(player) ? 2 : 1;
				Server.itemHandler.createGroundItem(player, casketId, x, y, amount, false, 0, true, "");
		}
