...
public class BotContent
{
		...
		public static void adjustWeaponAttackStyle(Player bot)
		{
				...
				if (RangedData.isWieldingMediumRangeRangedWeapon(bot) || RangedData.isWieldingShortRangeRangedWeapon(bot))
				{
		...
		{
				Movement.playerWalk(bot, 3089 + Misc.random(4), 3513 + Misc.random(3));
		}
				...
				CycleEventHandler.getSingleton().addEvent(bot, new CycleEvent()
				{
						...
						public void stop()
						{
								...
								if (!Area.inWilderness(bot))
								{
										...
										Movement.playerWalk(bot, coordinates[0] + Misc.random(10), coordinates[1] + Misc.random(6));
								}
						}
				}, tickDelay);
		}
		...
		public static void togglePrayer(final Player bot, final int prayerId, final boolean turnOn)
		{
				CycleEventHandler.getSingleton().addEvent(bot, new CycleEvent()
				{
						...
						public void stop()
						{
								...
								PrayerBook.activatePrayer(bot, prayerId);
						}
				}, Misc.random(2, 5));
		}
		...
		static int getPlayerMaxHitToEatAt(Player player)
		{
				...
				int max = 0;
				if (RangedData.isWieldingMediumRangeRangedWeapon(player) || RangedData.isWieldingShortRangeRangedWeapon(player))
				{
		...
		static void combatEvent(final Player bot)
		{
				...
				CycleEventHandler.getSingleton().addEvent(bot, new CycleEvent()
				{
						...
						public void execute(CycleEventContainer container)
						{
								...
								if (attacker != null)
								{
										if (attacker.prayerActive[ServerConstants.PROTECT_FROM_MELEE] || attacker.prayerActive[ServerConstants.PROTECT_FROM_MAGIC] || attacker.prayerActive[ServerConstants.PROTECT_FROM_RANGED])
										{
												if (!bot.prayerActive[bot.botLastDamageTakenType])
												{
		...
		private static void setCombatFollowing(Player bot, Player attacker)
		{
				...
				else
				{
						...
						bot.setUsingShortRangeRangedWeapon(false);
				}
				if (System.currentTimeMillis() - bot.botTimeSwitchedItem < 1190)
				{
						return;
		...
		private static void specialAttack(Player bot, Player attacker, boolean attackerExists)
		{
				...
				int botSpecAt = (int) (getBotInvisibleSpecialAttackWeaponMax(bot.botSpecialAttackWeapon, bot.getBaseDefenceLevel(), RangedData.wearingFullVoidRanged(bot) ? true : false) * getEatPercentage(attacker.getBaseDefenceLevel()));
				...
				if (!ItemAssistant.hasItemEquippedSlot(bot, bot.botSpecialAttackWeapon, ServerConstants.WEAPON_SLOT))
				{
						...
						if (bot.botShield > 0 && ItemAssistant.is2handed(ItemDefinition.getDefinitions()[bot.botSpecialAttackWeapon].name, bot.botSpecialAttackWeapon) && ItemAssistant.getFreeInventorySlots(bot) >= 1 || !ItemAssistant.is2handed(ItemDefinition.getDefinitions()[bot.botSpecialAttackWeapon].name, bot.botSpecialAttackWeapon) || bot.botShield <= 0)
						{
		...
		private static void wearBackPrimaryWeapon(Player bot, boolean wornItemThisTick)
		{
				...
				if (!bot.botWearingPrimaryWeapon)
				{
						...
						if (!ItemAssistant.hasItemEquippedSlot(bot, bot.botPrimaryWeapon, ServerConstants.WEAPON_SLOT))
						{
								...
								if (slot >= 0)
								{
										bot.botDebug.add("Here27: " + bot.botPrimaryWeapon + ", " + ItemAssistant.getItemSlot(bot, bot.botPrimaryWeapon) + ", " + ItemAssistant.getItemName(bot.playerItems[slot] - 1));
								}
								if (bot.botArrowSpecial > 0)
								{
						...
						if (!ItemAssistant.is2handed(ItemAssistant.getItemName(bot.botPrimaryWeapon), bot.botPrimaryWeapon))
						{
		...
		private static void lootingEvent(final Player bot)
		{
				...
				CycleEventHandler.getSingleton().addEvent(bot, new CycleEvent()
				{
						...
						public void stop()
						{
								...
								Movement.playerWalk(bot, bot.botEnemyDeathPosition[0], bot.botEnemyDeathPosition[1]);
								retreatToBank(bot, false);
						}
				}, Misc.random(5, 15));
		}
								...
								if (other.isCombatBot())
								{
										if (other.botPkType.isEmpty() && !other.getDead())
										{
												...
												CycleEventHandler.getSingleton().addEvent(other, new CycleEvent()
												{
														...
														public void stop()
														{
																Movement.playerWalk(other, (other.getX() - 3) + Misc.random(6), (other.getY() - 3) + Misc.random(6));
														}
												}, Misc.random(1, 5));
										}
								}
		...
		public static void regearStage1Event(final Player bot)
		{
				...
				CycleEventHandler.getSingleton().addEvent(bot, new CycleEvent()
				{
						...
						public void execute(CycleEventContainer container)
						{
								if (!Area.inWilderness(bot) && !bot.getDead())
								{
		...
		private static void interactWithBanker(Player bot)
		{
				...
				Movement.playerWalk(bot, 3092, 3515);
		...
		public static void gearUp(Player bot, boolean instantGearUp)
		{
				...
				QuickSetUp.heal(bot);
				QuickSetUp.spawnEquipment(bot, spawnEquipmentSet(bot, bot.botPkType), instantGearUp ? false : true);
				if (instantGearUp)
				{
						spawnOther(bot, true);
						int[] coordinates = getPkingCoordinates(bot.botPkType);
		...
		private static void findBotToAttack(Player bot)
		{
				...
				if (Misc.random(10) == 1)
				{
						for (int index = 0; index < ServerConstants.MAXIMUM_PLAYERS; index++)
						{
								...
								if (combatDif1 > bot.wildernessLevel || combatDif1 > loop.wildernessLevel)
								{
										continue;
								}
		...
		public static void handleSafing(final Player player)
		{
				...
				if (player.prayerActive[bot.botLastDamageTakenType] || player.prayerActive[ServerConstants.PROTECT_FROM_RANGED] || player.prayerActive[ServerConstants.PROTECT_FROM_MAGIC])
				{
						return;
				}
